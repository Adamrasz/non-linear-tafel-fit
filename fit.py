...
def bv_residuals(params, E, logI):
    ...
    i0 = 10**log_i0
    eta = E - Ecorr
    I_pred = i0 * (np.exp(alpha_a * F * eta / (R * T)) - np.exp(-alpha_c * F * eta / (R * T)))
...
def main(data_parent):
    summary = []
    # Traverse all subdirectories
    for root, dirs, files in os.walk(data_parent):
        for fname in files:
            if not fname.lower().endswith('.csv'): continue
            if 'tafel' in fname.lower(): continue
            csv_path = os.path.join(root, fname)
            df = pd.read_csv(csv_path)
            # only process Step number 3 scans
            if 'Step number' not in df.columns:
                continue
            ...
            res = least_squares(bv_residuals, p0, args=(E_fit, logI_fit), bounds=bounds, xtol=1e-12)
            ...
            resid = res.fun; chi2 = np.sum(resid**2); N = len(resid)
            chi_sqrtN = chi2 / np.sqrt(N)
            chi2_I2 = np.sum(resid**2 / (logI_fit**2 + 1e-12))
            eta = E_fit - Ecorr_opt
            I_model = i0_opt * (np.exp(alpha_a * F * eta / (R * T)) - np.exp(-alpha_c * F * eta / (R * T)))

            # 1) BV Tafel Fit Plot
            fig, ax = plt.subplots(figsize=(6,4), constrained_layout=True)
            ax.plot(eta, logI_fit + 3, 'k.', label='Data')
            ax.plot(eta, np.log10(np.abs(I_model)) + 3, 'r-', label='BV Fit')
            ax.axvline(0, ls='--', c='gray'); ax.axhline(log_i0_opt + 3, ls='--', c='gray')
            ax.set_xlabel('η (V)'); ax.set_ylabel('log10(|I|) (mA)')
            ax.set_title(fname)
            leg = ax.legend(loc='lower left', frameon=True)
            leg.get_frame().set_edgecolor('black'); leg.get_frame().set_linewidth(1)
            txt = AnchoredText(f"Ecorr={Ecorr_opt:.3f} V\nIcorr={i0_opt*1e6:.2f} µA",
                               loc='lower right', prop=dict(size=9), pad=0.3, frameon=True)
            txt.patch.set_edgecolor('black'); txt.patch.set_linewidth(1)
            ax.add_artist(txt)
            ax2 = ax.twinx()
            ax2.plot(eta, logI_fit + 3, '', alpha=0)
            ...
            ax.set_xlabel('η (V)'); ax.set_ylabel('Current (mA)')
            ...
            rect = Rectangle((bg_box[0], bg_box[1]), bg_box[2], bg_box[3],
            ...
            axins.set_ylim(min(Ic[mask_zoom]*1e3), max(Ia[mask_zoom]*1e3))
            axins.grid(True)
...
if __name__ == '__main__':
    # point to your top‐level experiments folder
    DATA_PARENT = os.getcwd()  # use current working directory by default
