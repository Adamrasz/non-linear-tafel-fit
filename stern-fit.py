...
best_a = {'length':0}
window_size = 5
# constrain start at i_corr/Ecorr crossing (eta=0)
idx0 = np.argmin(np.abs(eta_a))
for end_idx in range(idx0 + window_size, len(eta_a)):
    x = eta_a[idx0:end_idx]
    y = log_I_a[idx0:end_idx]
    slope, intercept, r, *_ = linregress(x, y)
    err = np.mean(np.abs(y - (slope*x + intercept)))
    if err < 0.05 and r*r > 0.995:
        region_length = end_idx - idx0
        if region_length > best_a.get('length', 0):
            best_a.update({'start': idx0, 'end': end_idx,
                          'slope': slope, 'intercept': intercept, 'r2': r*r,
                          'length': region_length})
# Fallback if no valid region found
def default_a():
    x = eta_a[idx0:idx0+window_size]
    slope, intercept, r, *_ = linregress(x, log_I_a[idx0:idx0+window_size])
    return {'start': idx0, 'end': idx0+window_size, 'slope': slope, 'intercept': intercept}
if 'start' not in best_a:
    fallback = default_a()
    best_a.update(fallback)
# Extract anodic baseline
...
# ----- Plot 3: Cathodic nonlinear fit on LSV data -----
plt.figure()
plt.plot(E_c, -I_c, 'ko', label='Cathodic I vs E')
I_c_nl = 10**bv_cath_tafel(E_c - Ecorr, i_corr_nl_c, alpha_c_nl)
plt.plot(E_c, I_c_nl, 'm--', label=f'Nonlinear Fit: βc={beta_c_nl:.1f} mV/dec')
plt.xlabel('Potential (V)'); plt.ylabel('Current (A)')
plt.title('Cathodic Nonlinear BV Fit'); plt.legend(); plt.grid(); plt.tight_layout()
plt.savefig('cathodic_nonlinear_fit.png')
plt.show()

# ----- Plot 4: Anodic & Cathodic full nonlinear fit on Tafel region -----
plt.figure()
