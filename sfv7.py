...
# =====================
def drop_initial_outlier(E: np.ndarray, I: np.ndarray, k: int = 12,
                         jump_ratio: float = 6.0, log_decades: float = 1.0):
    """
    Drop the first point ONLY if:
      (A) its jump to the 2nd point is huge relative to typical early point-to-point changes, AND
      (B) its log10(|I|) differs by >= log_decades from the median of the next k points.

    This targets true isolated "first point glitches" and avoids inconsistent behavior across files.
    """
    if len(I) < k + 2:
        return E, I, False

    # jump test: compare |I0-I1| to typical early |dI| (excluding the 0->1 transition)
    d0 = float(np.abs(I[0] - I[1]))
    dref = np.abs(np.diff(I[1:k+1]))
    med_dref = float(np.nanmedian(dref)) + 1e-30
    jr = d0 / med_dref

    # log magnitude test vs next k points
    ref_log = safe_log10_abs(I[1:k+1])
    med_log = float(np.nanmedian(ref_log))
    x0 = float(safe_log10_abs(np.array([I[0]]))[0])
    dev_dec = abs(x0 - med_log)

    if (jr >= jump_ratio) and (dev_dec >= log_decades):
        return E[1:], I[1:], True
...
def fit_lin_lin_ls(E_c, logI_c, E_a, logI_a, Ecorr0, E_bounds):
    ...
    def resid(p):
        ...
        # IMPORTANT: ensure cath points remain cath and anod points remain anod as Ec moves
...
def run_fits_for_window_asym(E_all: np.ndarray, I_sm_all: np.ndarray, Ecorr0: float, win_neg: float, win_pos: float):
    ...
    Branch split is by potential side (robust), not by current sign.
    """
    ...
    # robust split: by potential side of Ecorr0
...
def main():
    ...
    for path in paths:
        ...
        # Use SAFE folder name too (prevents any OS path issues)
        ...
        # ---- drop start-of-scan artifact if present (guard: don't break viability) ----
        E2, I2, dropped = drop_initial_outlier(E, I_raw, k=12, jump_ratio=6.0, log_decades=1.0)
        if dropped and len(E2) >= 20:
            E, I_raw = E2, I2
            if VERBOSE:
